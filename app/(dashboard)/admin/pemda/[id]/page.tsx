...
export default function EditPemdaPage() {
  const router = useRouter()
  const params = useParams()
  const id = params.id as string

  const [formData, setFormData] = useState<PemdaForm>({
    name: '',
    code: '',
    level: '',
    parentId: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = id === 'create' ? '/api/admin/pemda' : `/api/admin/pemda/${id}`
      ...
      console.error('Failed to save pemda:', error)
    } finally {
      setSaving(false)
    }
  }
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {id === 'create' ? 'Tambah' : 'Edit'} Pemda
          </h1>
          <p className="text-gray-600 mt-1">
            {id === 'create' ? 'Tambah' : 'Edit'} data pemerintah daerah
          </p>
        </div>
      </div>
