...
interface DashboardChartData {
  ...
  ringkasan_pembiayaan: ChartData[]
  pengeluaran_pembiayaan_persumberdana: ChartData[]
  penyertaan_modal_tertinggi: ChartData[]
}
...
export default function DashboardBelanjaPage() {
  ...
  const fetchChartData = async () => {
    ...
    try {
      const res = await fetch(`/api/dashboard/chart-data/pembiayaan${filterQueryString}`, { cache: 'no-store' })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
...
<div className="flex items-center justify-between">
    ...
    >
      ...
      <div className="relative bg-gray-50 px-4 sm:px-6 py-4 sm:py-6 rounded-lg w-full">
        ...
        <div className="overflow-hidden max-w-full" ref={emblaRef}>
          ...
          {chartData?.ringkasan_pembiayaan?.map((stat, index) => {
  ...
  <div className="flex items-center p-4">
    ...
    <div className="flex-1 min-w-0">
      ...
      ) : chartData && (chartData.pengeluaran_pembiayaan_persumberdana.length > 0 || chartData.penyertaan_modal_tertinggi.length > 0) ? (
        <>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 w-full">
            <div className="bg-white rounded-lg shadow p-4 sm:p-6">
              <BarChartDashboard data={chartData.pengeluaran_pembiayaan_persumberdana} mode="stacked" title="Pengeluaran Pembiayaan per Sumber Dana" nilai1Label="Anggaran" nilai2Label='Realisasi'    />
            </div>
            <div className="bg-white rounded-lg shadow p-4 sm:p-6">
              <BarChartDashboard data={chartData.penyertaan_modal_tertinggi} mode="stacked" title="10 Desa dengan Penyertaan Modal Tertinggi" nilai1Label="Anggaran" nilai2Label='Realisasi'    />
            </div>
          </div>  
