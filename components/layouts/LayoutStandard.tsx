      ...
      <header className={`fixed top-0 z-900 ${collapsed ? 'left-18' : 'left-72'} right-0 h-16 flex items-center justify-between px-6 ${activeTheme.topbar} ${activeTheme.text} transition-all duration-500`}>
        ...
        <div className="hidden md:block text-center">
          <p className="text-sm font-bold whitespace-nowrap">
            {session?.user?.pemdaName || process.env.NEXT_PUBLIC_PEMDA_NAME || 'Seluruh Indonesia'} - {session?.fiscalYear || new Date().getFullYear()}
          </p>
        </div>
...
<br/>
      ...
      {/* CONTENT */}
      <main className={`pt-20 transition-all duration-500 ${collapsed ? 'pl-18' : 'pl-72'} ${isActive('/integrated-maps') || isActive('/map') ? 'p-0' : 'p-8'}`}>
        <div className={isActive('/integrated-maps') || isActive('/map') ? '' : 'pl-6'}>{children}</div>
      </main>
